...
class UpdateVehicleUseCaseTest {
	...
	@Test
	void test_updateVehicle() throws BusinessException {
		...
		UpdateVehicleUseCase useCase = new UpdateVehicleUseCase(repository, mapper);
		VehicleResponse response = useCase.execute(VehicleMockUtil.vehicleDTO());
	...
	void test_notGetVehicle() {
		...
		UpdateVehicleUseCase useCase = new UpdateVehicleUseCase(repository, mapper);
		assertThrows(VehicleNotFoundException.class, () -> useCase.execute(VehicleMockUtil.vehicleDTO()));
	}
}
